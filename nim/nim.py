...
class NimAI():
    ...
    def best_future_reward(self, state):
        ...
        for action in Nim.available_actions(state):
            ...
            x.append(self.get_q_value(state, action))
        return(max(x))
    ...
    def choose_action(self, state, epsilon=True):
        ...
        def false(state):
            ...
            for i, j in self.q.items():
                if j == maximum and i[1][1] <= state[i[1][0]]:
                    return i[1]
        ...
        if self.epsilon == False:
            ...
            if len(self.q) != 0:
                x = random.randint(0,len(Nim.available_actions(state)) - 1)
                y = list(Nim.available_actions(state))[x]
                while True:
                    if y[1] <= state[y[0]]:
                        return random.choices([y, false(state)], [self.epsilon, 1-self.epsilon], k=1)[0]
            else:
                return false(state)

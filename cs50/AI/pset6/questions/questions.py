...
def top_files(query, files, idfs, n):
    ...
    if n > len(files):
        raise Exception("Sorry, number of return files greater than number of files")
    ...
    top = dict(itertools.islice(top.items(), n))
    return top
...
def top_sentences(query, sentences, idfs, n):
    ...
    if n > len(sentences):
        raise Exception("Sorry, number of return sentences greater than number of sentences")
    ...
    for f in sentences:
        ...
        for word in query:
            if word in sentences[f]:
                i += idfs[word]
                j += 1
        sen.update({f: [i, j/len(sentences[f])]})
    # Sorting in descending order with idf and qtd
